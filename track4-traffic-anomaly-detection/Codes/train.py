...
with tf.name_scope('dataset'):
    ...
    if (const.BLEND_MOTION == 0):
        ...
        train_dataset = train_loader(batch_size=batch_size, time_steps=2, num_pred=1)
    ...
    train_inputs_vis = train_videos_clips_tensor[..., (num_his - 2)*3:(num_his-1)*3]
    train_inputs_vis_2 = train_videos_clips_tensor[..., (num_his - 1) * 3:num_his * 3]
    ...
    if (const.BLEND_MOTION == 0):
        ...
        test_dataset = test_loader(batch_size=batch_size, time_steps=2, num_pred=1)
    ...
    test_inputs_vis = test_videos_clips_tensor[..., (num_his - 2)*3:(num_his-1)*3]
    test_inputs_vis_2 = test_videos_clips_tensor[..., (num_his - 1) * 3:num_his * 3]
...
with tf.variable_scope('generator', reuse=None):
    ...
    train_outputs = generator(train_inputs, layers=const.LAYERS, output_channel=3)
...
tf.summary.scalar(tensor=gdl_loss, name='gdl_loss')
tf.summary.image(tensor=train_inputs_vis, name='train_inputs')
tf.summary.image(tensor=train_inputs_vis_2, name='train_inputs_next')
tf.summary.image(tensor=train_outputs, name='train_outputs')
tf.summary.image(tensor=train_gt, name='train_gt')
tf.summary.image(tensor=test_inputs_vis, name='test_inputs')
tf.summary.image(tensor=test_inputs_vis_2, name='test_inputs_next')
...
with tf.Session(config=config) as sess:
    ...
    while _step < iterations:
        try:
            ...
            if _step % const.PER_STEP == 0:
                save(saver, sess, snapshot_dir, _step)
            toc = time.time()
            print("Training time for one step: ", toc-tic)
